...
from PyCatan.Classes.Constants import DevelopmentCardConstants as Dcc
import itertools
...
class EdoAgent(AgentInterface):
    ...
    def on_turn_start(self):
        ...
        if set(town_nodes).intersection(set(thief_nodes)):
            card = get_development_card(
                self.development_cards_hand.hand, Dcc.KNIGHT_EFFECT
            )
            if card:
    ...
    def on_moving_thief(self):
        ...
        return {
            "terrain": random.choice(list(others_terrain)),
            "player": random.choice(list({0, 1, 2, 3} - {self.id})),
        }

    def on_turn_end(self):
        card = get_development_card(
            self.development_cards_hand.hand, Dcc.ROAD_BUILDING_EFFECT
        )
        if card:
            return self.development_cards_hand.select_card(card)
        return None
    ...
    def on_build_phase(self, board_instance):
        ...
        if goal == "build_town" and self.hand.resources.has_more("town"):
            ...
            if len(valid_nodes):
                node_resources = [
                    get_node_resources(board_instance, node) for node in valid_nodes
                ]
                sorted_nodes = [
                    x
                    for _, x in sorted(
                        zip(node_resources, valid_nodes), key=lambda pair: pair[0]
                    )
                ]
            ...
            for end in road_ends:
                ...
                for adjacent in adjacent_roads:
                    materials = get_node_resources(
                        self.board, adjacent["finishing_node"]
                    )
    ...
    def on_game_start(self, board_instance):
        ...
        if current_town_nodes:
            current_resources = reduce(
                lambda x, y: madd(x, y),
                [
                    get_node_resources(board_instance, node)
                    for node in current_town_nodes
                ],
            )
        ...
        sorted_free = [
            x for _, x in sorted(zip(node_resources, free), key=lambda pair: pair[0])
        ]
        adjacent = board_instance.__get_adjacent_nodes__(
            sorted_free[0]
        )  # TODO: mejorable
    ...
    def on_road_building_card_use(self):
        ...
        for end in road_ends:
            ...
            if best_road:
                ret.update(
                    {
                        "node_id_2": best_road["starting_node"],
