...
dice_odds = {7: 6, 6: 5, 8: 5, 5: 4, 9: 4, 4: 3, 10: 3, 3: 2, 11: 2, 2: 1, 12: 1}
...
def get_roads(board: Board, player_id: int) -> Set[FrozenSet[int]]:
    ...
    for node in nodes:
        roads.update(
            {
                frozenset((node["id"], road["node_id"]))
...
def get_adjacent_road(board: Board, node_id: int, player_id: int) -> List:
    ...
    adjacent_roads = [
        {"starting_node": node_id, "finishing_node": node} for node in adjacent_nodes
    ]
    adjacent_roads += [
        {"starting_node": node, "finishing_node": node_id} for node in adjacent_nodes
    ]
