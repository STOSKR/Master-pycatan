...
class AlexPelochoJaimeAgent(AgentInterface):
    ...
    def on_game_start(self, board_instance):
        ...
        for node_id in possibilities:
            ...
            if node_id in self.board.nodes and "adjacent" in self.board.nodes[node_id]:
                for adj_node_id in self.board.nodes[node_id]["adjacent"]:
                    if (
                        adj_node_id in self.board.nodes
                        and "number" in self.board.nodes[adj_node_id]
                    ):
                        if self.board.terrain[self.board.nodes[adj_node_id]["terrain"]][
                            "probability"
                        ] in [6, 8]:
                            # Encontrar el siguiente número más cercano a 7
                            adjacent_numbers = [
                                self.board.nodes[adj_node_id]["number"]
                                for adj_node_id in self.board.nodes[node_id]["adjacent"]
                            ]
                            closest_to_7 = min(
                                adjacent_numbers, key=lambda x: abs(x - 7)
                            )

                            # Elegir el nodo si está desocupado
                            if closest_to_7 not in [
                                self.board.nodes[adj_node_id]["number"]
                                for adj_node_id in self.board.nodes[node_id]["adjacent"]
                            ]:
        ...
        if (
            chosen_node_id in self.board.nodes
            and "adjacent" in self.board.nodes[chosen_node_id]
        ):
    ...
    def on_build_phase(self, board_instance):
        ...
        if len(self.development_cards_hand.hand):
            for i, card in enumerate(self.development_cards_hand.hand):
                if (
                    card.effect == DevelopmentCardConstants.YEAR_OF_PLENTY_EFFECT
                    or card.effect == DevelopmentCardConstants.ROAD_BUILDING_EFFECT
                ):
                    return self.development_cards_hand.select_card(i)
    ...
    def on_having_more_than_7_materials_when_thief_is_called(self):
        ...
        if hand_resources.has_more("city"):
            ...
            while self.hand.get_total() > 7:
                ...
                available_indices = [
                    idx for idx, count in enumerate(self.hand.resources) if count > 0
                ]
