...
class PabloAleixAlexAgent(AgentInterface):
    ...
    def on_moving_thief(self):
        ...
        for terrain in self.board.terrain:
            if not terrain["has_thief"] and (
                terrain["probability"] == 6 or terrain["probability"] == 8
            ):
                nodes = self.board.__get_contacting_nodes__(terrain["id"])
                has_own_town = any(
                    self.board.nodes[node_id]["player"] == self.id for node_id in nodes
                )
                enemy = next(
                    (
                        self.board.nodes[node_id]["player"]
    ...
    def on_commerce_phase(self):
        ...
        if self.town_number >= 1 and self.hand.resources.has_more(BuildConstants.CITY):
            ...
            total_given_materials = (
                5 - cereal_hand - mineral_hand - wood_hand - clay_hand - wool_hand
            )
    ...
    def on_build_phase(self, board_instance):
        ...
        def calculate_probability_sum(node_id):
            return sum(
                self.board.terrain[terrain_piece_id]["probability"]
    ...
    def on_road_building_card_use(self):
        ...
        def calculate_node_score(node):
            return (
                node["probability"]
                + len(node["adjacent"])
                - 2 * self.board.count_opponent_towns(node["starting_node"])
            )
        ...
        if len(valid_nodes) > 1:
            return {
                ...
                "road_to": random.choice(
                    valid_nodes[0]["adjacent"]
                ),  # Choose a random adjacent node
                "node_id_2": valid_nodes[1]["starting_node"],
                "road_to_2": random.choice(
                    valid_nodes[1]["adjacent"]
                ),  # Choose a random adjacent node
            }
