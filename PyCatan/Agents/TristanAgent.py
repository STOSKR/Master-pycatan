...
from PyCatan.Classes.Constants import MaterialConstants, BuildConstants
from PyCatan.Classes.Constants import DevelopmentCardConstants
...
def evaluate_node(node_id, board_instance):
    numbers = [
        board_instance.terrain[v]["probability"]
        for v in board_instance.nodes[node_id]["contacting_terrain"]
    ]
    ...
    resources = [
        board_instance.terrain[v]["terrain_type"]
        for v in board_instance.nodes[node_id]["contacting_terrain"]
    ]
...
class TristanAgent(AgentInterface):
    ...
    def on_moving_thief(self):
        ...
        for node in self.board.terrain[terrain]["contacting_nodes"]:
            if (
                self.board.nodes[node]["player"] != -1
                and self.board.nodes[node]["player"] != self.id
            ):
    ...
    def calc_give_offer(
        self, need, wool=0, wood=0, cereal=0, mineral=0, clay=0
    ) -> Materials:
        TRADE_FACTOR = 2
        ...
        result.add_from_id(
            ...
            max(0, min(need * TRADE_FACTOR, self.hand.resources.wool - wool)),
        )

        if self.total_material(result) < need * TRADE_FACTOR:
            result.add_from_id(
                MaterialConstants.WOOD,
                max(0, min(need * TRADE_FACTOR, self.hand.resources.wood - wood)),
            )

        if self.total_material(result) < need * TRADE_FACTOR:
            result.add_from_id(
                MaterialConstants.CEREAL,
                max(0, min(need * TRADE_FACTOR, self.hand.resources.cereal - cereal)),
            )

        if self.total_material(result) < need * TRADE_FACTOR:
            result.add_from_id(
                MaterialConstants.MINERAL,
                max(0, min(need * TRADE_FACTOR, self.hand.resources.mineral - mineral)),
            )

        if self.total_material(result) < need * TRADE_FACTOR:
            result.add_from_id(
                MaterialConstants.CLAY,
                max(0, min(need * TRADE_FACTOR, self.hand.resources.clay - clay)),
            )
    ...
    def on_commerce_phase(self):
        ...
        # Missing wood
        if (
            self.hand.resources.wood == 0
            and self.hand.resources.clay >= 1
            and self.hand.resources.wool >= 1
            and self.hand.resources.cereal >= 1
        ):
        ...
        if (
            self.hand.resources.wood >= 1
            and self.hand.resources.clay == 0
            and self.hand.resources.wool >= 1
            and self.hand.resources.cereal >= 1
        ):
        ...
        if (
            self.hand.resources.wood >= 1
            and self.hand.resources.clay >= 1
            and self.hand.resources.wool == 0
            and self.hand.resources.cereal >= 1
        ):
        ...
        if (
            ...
            and self.hand.resources.clay >= 1
            and self.hand.resources.wool >= 1
            and self.hand.resources.cereal == 0
        ):
    ...
    def on_build_phase(self, board_instance):
        ...
        if (
            ...
            and len(self.board.valid_city_nodes(self.id)) > 0
        ):
        ...
        if len(self.development_cards_hand.hand):
            for i in range(0, len(self.development_cards_hand.hand)):
                ...
                if (
                    ...
                    and len(self.board.valid_road_nodes(self.id)) >= 2
                ):
                    return self.development_cards_hand.select_card(i)
        ...
        # carretera
        if (
            self.hand.resources.has_more(BuildConstants.ROAD)
